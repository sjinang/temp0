...
uchar* eval_all_ss(ZBuff *zbuff){
  ...
  for(int y=0 ;  y<h ; y++ ) {
    for(int x=0 ; x<w ; x++ ) {
      ...
      //   long pixel = 0;
      //   printf("x: %d, y: %d w: %d\n",x, y, w);
      //   for(int ssy = 0; ssy < zbuff->config.ss_w; ssy++){
      //     for(int ssx = 0; ssx < zbuff->config.ss_w; ssx++){
      //       printf("ssx: %d, ssy: %d, ss: %d  || ", ssx, ssy, zbuff->frame_buffer[ idx_f(zbuff, x , y , ssx , ssy , 0 ) ]);
      //       pixel += zbuff->frame_buffer[ idx_f(zbuff, x , y , ssx , ssy , 0 ) ];
      //     }
      //     printf("\n");
      //   }
      //   pixel = pixel/(zbuff->config.ss_w*zbuff->config.ss_w);
      //   pixel = pixel/256;
      //   printf("pixel R=%ld\n",pixel);
      // }
    }
...
void process_fragment(ZBuff *zbuff, Sample hit_location, Sample subsample, Fragment f){
  // if(hit_location.x==266 && hit_location.y==335 && subsample.x==3 && subsample.y==0){
  //   printf("R:%d Z:%#08x\n", f.R, f.z);
  // }
