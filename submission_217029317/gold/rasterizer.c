...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  bool backface_cull_valid = 0 > ((triangle.v[1].x - triangle.v[0].x) * (triangle.v[2].y - triangle.v[1].y) - (triangle.v[2].x - triangle.v[1].x) * (triangle.v[1].y - triangle.v[0].y));
  
  
  // initialize bounding box to first vertex

  int min_x = min(min(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  int min_y = min(min(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);

  int max_x = max(max(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  int max_y = max(max(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);

  // round down to subsample grid
  bbox.lower_left.x = floor_ss(min_x, config.r_shift, config.ss_w_lg2);
  bbox.lower_left.y = floor_ss(min_y, config.r_shift, config.ss_w_lg2);

  bbox.upper_right.x = floor_ss(max_x, config.r_shift, config.ss_w_lg2);
  bbox.upper_right.y = floor_ss(max_y, config.r_shift, config.ss_w_lg2);
  ...
  bbox.valid = backface_cull_valid && (bbox.lower_left.x <= bbox.upper_right.x) && (bbox.lower_left.y <= bbox.upper_right.y);
