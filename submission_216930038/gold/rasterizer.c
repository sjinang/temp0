...
{
  // START CODE HERE
  // Multiplying val by 2^ss_w_lg2
  int fixedVal = val << ss_w_lg2;
  // Dividing val by 2^r_shift to floor the value (decimal points are removed in int).
  fixedVal = fixedVal >> r_shift;
  // Multiplying val by 2^r_shift to get the floored value.
  fixedVal = fixedVal << r_shift;
  // Divide val by 2^ss_w_lg2 to get the original but floored value back.
  fixedVal = fixedVal >> ss_w_lg2;
  return fixedVal;
  // END CODE HERE
}
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  if((bbox.upper_right.x < bbox.lower_left.x) || (bbox.upper_right.y < bbox.lower_left.y)){
    bbox.valid = false;
  }
  ...
  //     s.x = sl_x;
  //     s.y = sl_y;
  //     Sample j = jitter_sample(s, config.ss_w_lg2);
  //     s.x = sl_x + j.x;
  //     s.y = sl_y + j.y;
