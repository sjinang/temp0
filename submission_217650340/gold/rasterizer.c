...
int floor_ss(int val, int r_shift, int ss_w_lg2)
{
  ...
  int binary_val_to_invert = 0;
  for (int i = 0; i < r_shift - ss_w_lg2; i++) {
    binary_val_to_invert = binary_val_to_invert << 1 | 0b1;
  }

  val &= ~binary_val_to_invert;
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  bbox.valid = (bbox.lower_left.x <= screen.width) && (bbox.upper_right.x <= screen.width) && (bbox.lower_left.x <= bbox.upper_right.x) && (bbox.lower_left.y <= screen.height) && (bbox.upper_right.y <= screen.height) && (bbox.lower_left.y <= bbox.upper_right.y);
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  // Shift vertices so sample is the origin
  ...
  // Get distances
  // Assuming everything has the sample shift, we shouldn't have to make any modifications
  int zero_one_edge = v0_x * v1_y - v1_x * v0_y;
  int one_two_edge = v1_x * v2_y - v2_x * v1_y;
  int two_zero_edge = v2_x * v0_y - v0_x * v2_y;

  // Check if point is on the right side
  isHit = zero_one_edge <= 0 && one_two_edge < 0 && two_zero_edge <= 0;
