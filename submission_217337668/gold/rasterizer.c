...
{
  int r_trunc = r_shift - ss_w_lg2;
  return (val >> r_trunc) << r_trunc;
}
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  // initialize bounding box to min/max x/y coordinates of vertices
  bbox.lower_left.x = min(min(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  bbox.lower_left.y = min(min(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);
  bbox.upper_right.x = max(max(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  bbox.upper_right.y = max(max(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);
  ...
  bool backward_facing =  (triangle.v[1].x - triangle.v[0].x) * (triangle.v[2].y - triangle.v[1].y) > 
                          (triangle.v[2].x - triangle.v[1].x) * (triangle.v[1].y - triangle.v[0].y);
  bbox.valid =  (bbox.upper_right.x >= 0) && 
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  return (dist0 <= 0) && (dist1 < 0) && (dist2 <= 0);
}
...
int rasterize_triangle(Triangle triangle, ZBuff *z, Screen screen, Config config)
{
  ...
  // uncomment this line to mimic bubble smashing
  // while (!bbox.valid) bbox = get_bounding_box(triangle, screen, config); 
