...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  BoundingBox bbox;
  int cull;
  cull = ((triangle.v[1].x - triangle.v[0].x)*(triangle.v[2].y - triangle.v[1].y) <= (triangle.v[1].y - triangle.v[0].y)*(triangle.v[2].x - triangle.v[1].x)) ? 1 : 0;
  ...
  bbox.valid = cull && (bbox.lower_left.x >= 0) && (bbox.lower_left.y >= 0) && (bbox.upper_right.x < screen.width ) && (bbox.upper_right.y < screen.height);
  // END CODE HERE
  return bbox;
}
