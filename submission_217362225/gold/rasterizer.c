...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  bbox.upper_right.x = min(bbox.upper_right.x, screen.width << config.r_shift);
  bbox.upper_right.y = min(bbox.upper_right.y, screen.height << config.r_shift);  

  // check if bbox is valid
  bbox.valid = (bbox.lower_left.x <= bbox.upper_right.x) && (bbox.lower_left.y <= bbox.upper_right.y);

  // do backface culling:
  Vertex2D triangle_edge[2];
  triangle_edge[0].x = triangle.v[1].x - triangle.v[0].x;
  triangle_edge[0].y = triangle.v[1].y - triangle.v[0].y;

  triangle_edge[1].x = triangle.v[2].x - triangle.v[1].x;
  triangle_edge[1].y = triangle.v[2].y - triangle.v[1].y;

  int res;
  res = triangle_edge[0].x * triangle_edge[1].y - triangle_edge[1].x * triangle_edge[0].y;
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    triangle.v[i].x = triangle.v[i].x - sample.x;
    triangle.v[i].y = triangle.v[i].y - sample.y;
  }

  int res[3];

  for (int i = 0; i < 3; i++) {
    res[i] = edge_equation(triangle.v[i].x, triangle.v[i].y, triangle.v[(i+1)%3].x, triangle.v[(i+1)%3].y);
  }

  isHit = (res[0] <= 0 && res[1] < 0 && res[2] <= 0);
  // END CODE HERE
