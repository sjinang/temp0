...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  int backface_check = (triangle.v[1].x - triangle.v[0].x)*(triangle.v[2].y - triangle.v[1].y) - (triangle.v[2].x - triangle.v[1].x)*(triangle.v[1].y - triangle.v[0].y);
  ...
  bbox.valid = (bbox.upper_right.x >= 0) && (bbox.upper_right.y >= 0) && (bbox.lower_left.x < screen.width ) && (bbox.lower_left.y < screen.height) && (backface_check <= 0);
