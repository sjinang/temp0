...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  //  shifted_triangle.v[vertex].x = triangle.v[vertex].x - triangle.v[0].x;
  //  shifted_triangle.v[vertex].y = triangle.v[vertex].y - triangle.v[0].y;
  //}
  //int distance;
  //distance = (shifted_triangle.v[1].x * shifted_triangle.v[2].y) -
  //               (shifted_triangle.v[2].x * shifted_triangle.v[1].y);
  //bool good_triangle;
  //good_triangle = (distance < 0);
  //bbox.valid = (distance < 0) && (bbox.upper_right.x >= 0) && (bbox.upper_right.y >= 0) && (bbox.lower_left.x < screen.width ) && (bbox.lower_left.y < screen.height);
