...
{
  // START CODE HERE
  int rounded_val = (val >> (r_shift - ss_w_lg2)<< (r_shift - ss_w_lg2));
  return rounded_val;
  // END CODE HERE
}
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  // START CODE HERE
  // initialize bounding box to first vertex & iterate over remaining vertices
  BoundingBox bbox;
  int minX, minY, maxX, maxY;
  int is_back;
  int v1_x = triangle.v[1].x - triangle.v[0].x;
  int v1_y = triangle.v[1].y - triangle.v[0].y;
  int v2_x = triangle.v[2].x - triangle.v[0].x;
  int v2_y = triangle.v[2].y - triangle.v[0].y;
  Vertex2D lower_left, upper_right;
  minX = min(min(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  maxX = max(max(triangle.v[0].x, triangle.v[1].x), triangle.v[2].x);
  minY = min(min(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);
  maxY = max(max(triangle.v[0].y, triangle.v[1].y), triangle.v[2].y);
  // round down to subsample grid
  lower_left.x = floor_ss(minX, config.r_shift, config.ss_w_lg2);
  lower_left.y = floor_ss(minY, config.r_shift, config.ss_w_lg2);
  upper_right.x = floor_ss(maxX, config.r_shift, config.ss_w_lg2);
  upper_right.y = floor_ss(maxY, config.r_shift, config.ss_w_lg2);

  // clip to screen
  lower_left.x = max(lower_left.x, 0);
  lower_left.y = max(lower_left.y, 0);
  upper_right.x = min(upper_right.x, screen.width);
  upper_right.y = min(upper_right.y, screen.height);
  bbox.lower_left = lower_left;
  bbox.upper_right = upper_right;
  // check if bbox is valid
  is_back = (v1_x * v2_y - v2_x * v1_y > 0);
  
  if(lower_left.x > screen.width || lower_left.y > screen.height ||
     upper_right.x < 0 || lower_left.y < 0 || is_back){
    bbox.valid = 0;
  } else {
    bbox.valid = 1;
  }
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  // START CODE HERE
    int s_x = sample.x;
    int s_y = sample.y;
    int v0_x = triangle.v[0].x - s_x;
    int v0_y = triangle.v[0].y - s_y;
    int v1_x = triangle.v[1].x - s_x;
    int v1_y = triangle.v[1].y - s_y;
    int v2_x = triangle.v[2].x - s_x;
    int v2_y = triangle.v[2].y - s_y;
