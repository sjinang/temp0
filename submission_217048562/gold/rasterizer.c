...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  // check backface
  int no_back_face = ((triangle.v[1].x - triangle.v[0].x)*(triangle.v[2].y - triangle.v[1].y) - (triangle.v[2].x - triangle.v[1].x)*(triangle.v[1].y - triangle.v[0].y)) < 0;

 
  // check if bbox is valid
  bbox.valid = (bbox.upper_right.x >= 0) && (bbox.upper_right.y >= 0) && (bbox.lower_left.x < screen.width ) && (bbox.lower_left.y < screen.height) && no_back_face;
