...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  // calculate backface - if (x2, y2) to the left of (x0, y0) -> (x1, y1), then it's always backface
  int comp = (triangle.v[0].x - triangle.v[2].x) * (triangle.v[1].y - triangle.v[2].y) - (triangle.v[1].x - triangle.v[2].x) * (triangle.v[0].y - triangle.v[2].y);
  bool is_backface = comp > 0;
  ...
  bbox.valid = (bbox.upper_right.x >= 0) && (bbox.upper_right.y >= 0) && (bbox.lower_left.x < screen.width ) && (bbox.lower_left.y < screen.height) && !is_backface;
