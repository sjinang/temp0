...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  bbox.upper_right.y = min(bbox.upper_right.y, screen.width);
  ...
  bool valid_x = bbox.upper_right.x >= bbox.lower_left.x;
  bool valid_y = bbox.upper_right.y >= bbox.lower_left.y;
  bbox.valid = valid_x && valid_y;
  }
  // END CODE HERE
  return bbox;
}
