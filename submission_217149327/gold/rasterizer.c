...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  for(int i = 1; i < 3; i++) 
  {
    bbox.lower_left.x = min(triangle.v[i].x, bbox.lower_left.x);
    bbox.lower_left.y = min(triangle.v[i].y, bbox.lower_left.y);
    bbox.upper_right.x = max(triangle.v[i].x, bbox.upper_right.x);
    bbox.upper_right.y = max(triangle.v[i].y, bbox.upper_right.y);
  }
  ...
  // Early BackFace Culling using Edge Function
  bbox.valid = (triangle.v[2].x - triangle.v[0].x) * (triangle.v[1].y - triangle.v[0].y) -
               (triangle.v[2].y - triangle.v[0].y) * (triangle.v[1].x - triangle.v[0].x) >= 0; 
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    triangle.v[i].x -= sample.x;
    triangle.v[i].y -= sample.y;
  }

  int b[3];
  b[0] = triangle.v[0].x * triangle.v[1].y - triangle.v[1].x * triangle.v[0].y;
  b[1] = triangle.v[1].x * triangle.v[2].y - triangle.v[2].x * triangle.v[1].y;
  b[2] = triangle.v[2].x * triangle.v[0].y - triangle.v[0].x * triangle.v[2].y;

  isHit = (b[0] <= 0 && b[1] < 0 && b[2] <= 0);
  // END CODE HERE
